...
2. **Start the issue.** Claude Code: `/otta:start`; Codex: `$otta-start`. Pass the issue number. This seeds `.pr-body.md` from the issue's ACs with the canonical acceptance block. If the issue has no checkboxes, stop and get real ACs added first.
...
4. **Ship through the gate.** Claude Code: `/otta:ship`; Codex: `$otta-ship`. This runs the local gate (mirrors the Pulse merge gates), then opens the PR with the seeded body. The body must carry:
...
Use the setup entry point — Claude Code: `/otta:setup`; Codex: `$otta-setup`. It installs the pre-push gate hook and walks through installing the Otta Pulse GitHub App. A pushed change that skips the loop fails the gate locally before it ever reaches CI.
