...
#   1. the workflow still runs shellcheck as a BLOCKING error-severity gate
...
# 1. The workflow invokes shellcheck at error severity.
grep -qE 'shellcheck .*--severity=error' "$CI" \
  || fail "ci.yml must run shellcheck with --severity=error as a blocking gate"

# 2. That invocation must NOT be neutered with `|| true` on the same line —
#    a gate that cannot fail is not a gate.
if grep -E 'shellcheck .*--severity=error' "$CI" | grep -q '|| *true'; then
  fail "the error-severity shellcheck run must not be suppressed with '|| true'"
fi

# 3. A separate non-blocking report surfaces warning/info/style findings.
grep -qE 'shellcheck .*--severity=(style|info|warning)' "$CI" \
...
# 6. The tree is clean at error severity. Skip (don't fail) when shellcheck is
...
cd "$REPO"
if ! out="$(shellcheck --severity=error --format=gcc scripts/*.sh hooks/*.sh 2>&1)"; then
  echo "✗ shellcheck reports error-severity findings:" >&2
  printf '%s\n' "$out" >&2
  exit 1
fi

echo "✓ shellcheck gate is wired in ci.yml and the tree is clean at error severity"
