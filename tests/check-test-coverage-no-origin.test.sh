...
R="$(mk_no_origin_repo)"; ( cd "$R"; bash "$SCRIPT" >/dev/null 2>&1 ); check "no-origin + test in diff → pass (no crash)" 0 "$?"

# 2. Change WITHOUT a test and no [test-impractical] → clean exit 1 (not a crash/137).
R="$(mk_no_origin_repo)"; ( cd "$R"; rm a.test.ts; git commit -qam "drop test"; printf '# x\n' > .pr-body.md; bash "$SCRIPT" >/dev/null 2>&1 ); check "no-origin + no test → clean fail (exit 1)" 1 "$?"

# 3. Empty diff (HEAD vs HEAD) → "no changes to gate yet" exit 0.
R="$(mk_no_origin_repo)"; ( cd "$R"; bash "$SCRIPT" HEAD >/dev/null 2>&1 ); check "explicit empty diff → exit 0" 0 "$?"
