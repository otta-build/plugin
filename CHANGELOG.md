## 0.16.0 — harness-agnostic telemetry adapters: Codex CLI + Gemini CLI (phase 1)
- **feat(telemetry):** `scripts/otta-codex-setup.sh <owner/repo> <pulse-token>` writes a `[otel]` table to `~/.codex/config.toml` (merge-into-existing, never clobber, idempotent); sets `logs_endpoint`, `headers` (x-pulse-token), `resource_attributes` (repo + harness=codex); ensures `~/.codex/config.toml` is appended to `~/.gitignore_global` (warns if absent); prints consent disclosure; exits 0. TOML merge via pure-Python text manipulation — zero external dependencies. (#30)
- **feat(telemetry):** `scripts/otta-gemini-setup.sh <owner/repo> <pulse-token>` writes `.gemini/settings.json` (telemetry.endpoint → local OTel Collector at localhost:4318) and `otel-collector-config.yaml` sidecar config. Collector adds `x-pulse-token` header (via `${env:OTTA_PULSE_TOKEN}` env substitution — token never in any file), `repo` + `harness=gemini` resource attrs, and `cost_usd` transform from tokens. Prints consent disclosure + docker start instructions. `.gemini/settings.json` gitignored; `otel-collector-config.yaml` committable. (#30)
- **test:** `tests/otta-codex-setup.test.sh` (8 checks: write, URL override, merge preserves extra keys, idempotent, gitignore_global, warn when absent, consent disclosure, usage guard). `tests/otta-gemini-setup.test.sh` (13 checks: create both files, telemetry endpoint, collector yaml sections+port, repo+harness attrs, token env substitution safety, cost_usd, pulse URL default+override, merge, idempotent, gitignore, no staged token, consent+sidecar instructions, usage guard).
- **version:** 0.15.0 → 0.16.0. No regression of existing test suite.
